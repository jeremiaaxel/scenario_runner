...
DEFAULT_TARGET_DIR = f"{os.getenv('SCENARIO_RUNNER_ROOT', './')}/srunner/examples"

class CustomScenarioConfigurationParser(object):

    """
    Class providing access to parser methods for scenario configuration files (*.xml)
    Modified from ScenarioConfigurationParser to allow for specifying a target directory
    """

    def __init__(self, target_dir: str = DEFAULT_TARGET_DIR) -> None:
        if target_dir.startswith("./"):
            target_dir = f"{os.getenv('SCENARIO_RUNNER_ROOT', './')}/{target_dir[2:]}"

        self.target_dir = target_dir

    def parse_scenario_configuration(self, scenario_name: str, config_file_name: str):
        """
        Parse all scenario configuration files at target_dir and the additional
        ...
        list_of_config_files = glob.glob(f"{self.target_dir}/*.xml")
    ...
    def get_list_of_scenarios(self, config_file_name):
        """
        Parse *all* config files and provide a list with all scenarios @return
        """

        list_of_config_files = glob.glob(f"{self.target_dir}/*.xml")
        list_of_config_files += glob.glob(f"{self.target_dir}/*.xosc")
