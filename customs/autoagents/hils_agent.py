    ...
    def _setup_sensors(self):
        ...
        for sensor in self.sensors():
            if sensor["type"] == "sensor.camera.rgb":
                ...
                self._camera_handler = CameraHandler(zmq_host, port, zmq_context, is_time_logging_enabled=False)
            elif sensor["type"] == "sensor.lidar.ray_cast":
                port = os.getenv("ZMQ_LIDAR_PORT", 5558)
                self._lidar_handler = LidarHandler(zmq_host, port, zmq_context, is_time_logging_enabled=False)
            elif sensor["type"] == "sensor.other.gnss":
                port = os.getenv("ZMQ_GNSS_PORT", 5557)
                self._gnss_handler = GnssHandler(zmq_host, port, zmq_context, is_time_logging_enabled=False)
            else:
        ...
        self._control_receiver = ControlHandler(
            zmq_host, port, self._on_vehicle_control, zmq_context, is_time_logging_enabled=False
        )
